...
template_path = "frames/images/back_right/frame_0000.jpg"
...
frame_pattern = "frames/images/back_right/frame_*.jpg"
...
def generate_labels(dst, count, frame_file): # Get dst and count from the template matching function
    if dst is not None:
        ...
        if x_top < 0: x_top = 0
        if y_top < 0: y_top = 0
        if x_bottom <= x_top: x_bottom = x_top + 100  # minimum width
        if y_bottom <= y_top: y_bottom = y_top + 100  # minimum height
    else:
        # Fallback: use the entire template size as bounding box
        x_top, y_top = 0, 0
        x_bottom, y_bottom = vo_w, vo_h
    
    name = str("Image"+str(count)+".txt")
    with open(name, "w+") as f:
        f.write(str(x_top)+" "+str(y_top)+" "+str(x_bottom)+" "+str(y_bottom)+"\n")
    f.close()
    print(f"Created {name} for {frame_file}")
...
for frame_file in frame_files:
    ...
    dst = None  # Initialize dst as None
    ...
    if desc_grayframe is not None:
        ...
        if (len(good)>7): # Threshold for number of matched features
            ...
            print(f"Good match found for frame {count}: {frame_file}")
        else:
            print(f"Insufficient good matches ({len(good)}) for {frame_file} - using fallback")
    else:
        print(f"No features found in {frame_file} - using fallback")
    
    # Always save the frame and generate a label
    cv2.imwrite(os.path.join(output_dir, f"Image{count}.jpg"), frame)
    generate_labels(dst, count, frame_file)
    count += 1
